...
/proc/sanitize(var/input, var/max_length_char = MAX_MESSAGE_LEN, var/encode = 1, var/trim = 1, var/extra = 1)
	...
	if(max_length_char)
		//testing shows that just looking for > max_length_char alone will actually cut off the final character if message is precisely max_length_char, so >= instead
		if(length_char(input) >= max_length_char)
			var/overflow = ((length_char(input)+1) - max_length_char)
